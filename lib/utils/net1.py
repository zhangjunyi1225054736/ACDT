...
def smooth_l1_loss(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, alpha=0.5, gamma=1.5, beta=1.0):
    """
    SmoothL1(x) = 0.5 * x^2 / beta      if |x| < beta
                  |x| - 0.5 * beta      otherwise.
    1 / N * sum_i alpha_out[i] * SmoothL1(alpha_in[i] * (y_hat[i] - y[i])).
    ...
    abs_in_box_diff = torch.abs(in_box_diff)
    smoothL1_sign = (abs_in_box_diff < beta).detach().float()
    in_loss_box = smoothL1_sign * (alpha / b * (b * abs_in_box_diff + 1) * torch.log(b * abs_in_box_diff / beta + 1) - alpha * abs_in_box_diff) + \
                  (1 - smoothL1_sign) * (gamma * abs_in_box_diff + gamma / b - alpha * beta)

...
def _sample_rois(roidb, im_scale, batch_idx):
    ...
    bg_inds_region = [np.where((max_overlaps < cfg.TRAIN.BG_THRESH_region[i+1]) &
                       (max_overlaps >= cfg.TRAIN.BG_THRESH_region[i]))[0] for i in range(len(cfg.TRAIN.BG_THRESH_region)-1) ]

    # Compute number of background RoIs to take from this image (guarding
    # against there being fewer than desired)
    bg_rois_per_this_image = rois_per_image - fg_rois_per_this_image
    bg_inds = []
    for t in range(len(bg_inds_region)):
        i = len(bg_inds_region)-t-1
        expected_per_region = np.minimum(int(bg_rois_per_this_image * cfg.TRAIN.BG_THRESH_fraction[i]), bg_inds_region[i].size)
        bg_rois_per_this_image -= expected_per_region
        if bg_inds_region[i].size>0:
            bg_inds.append(npr.choice(
                bg_inds_region[i], size=expected_per_region, replace=False))
    # for x in bg_inds:
    #     print(x.size, end=" ")
    # print()
    if len(bg_inds)>0:
        bg_inds = np.hstack(bg_inds)
    else:

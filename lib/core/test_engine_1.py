...
def test_net_on_dataset(
    ...
    #dataset_name = 
    #dataset = JsonDataset(dataset_name, args.test_id)
    test_timer = Timer()
    test_timer.tic()
    if multi_gpu:
...
def write_result_test(file_name, result):
    if not os.path.exists("./test_result"):
           os.mkdir("./test_result")
    
    filename = "./test_result/"+file_name+".txt"
    print(filename)
    with open(filename, 'w') as file_object:
         for i, cls in enumerate(result[1:11]):
             print(i)
             for line in cls:
                 x = line[0]
                 y = line[1]
                 ...
                 truncation = -1
                 occlusion = 0
                 box = str(x)+","+str(y)+","+str(w)+","+str(h)+","+str(score)+","+str(object_category)+","+str(truncation)+","+str(occlusion)
                 file_object.write(box+'\n')    
         exit() 
...
def test_net(
    ...
    #roidb, dataset, start_ind, end_ind, total_num_images = get_roidb_and_dataset(
        #dataset_name, proposal_file, ind_range, args.test_id
    #)
    ...
    image_dir =  "/data/zhangjunyi/drone-object-detection/VisDrone2019-DET-test-challenge/test-challenge.txt" 
    image_list = open(image_dir).readlines()
    root_dir = "/data/zhangjunyi/drone-object-detection/VOC2007/JPEGImages/" 
    ...
    for i, name in enumerate(image_list):
        if cfg.TEST.PRECOMPUTED_PROPOSALS:
            ...
            box_proposals = None
        print(str(i)+":"+root_dir+name.strip()+".jpg")
        im = cv2.imread(root_dir+name.strip()+".jpg")
        cls_boxes_i, cls_segms_i, cls_keyps_i = im_detect_all(model, im, box_proposals, timers, name.strip())

        print("cls_boxes_i shape:", np.array(cls_boxes_i).shape)
        write_result_test(name.strip(), cls_boxes_i)    
        #for  in 
        #print(cls_boxes_i[4])
...
def get_roidb_and_dataset(dataset_name, proposal_file, ind_range, test_id=-1):
    """Get the roidb for the dataset specified in the global cfg. Optionally
    restrict it to a range of indices if ind_range is a pair of integers.
    """
    dataset = JsonDataset(dataset_name, test_id)
    if cfg.TEST.PRECOMPUTED_PROPOSALS:

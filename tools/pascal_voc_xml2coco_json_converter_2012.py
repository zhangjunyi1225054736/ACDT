...
class voc2coco:
    def __init__(self, devkit_path=None, year=None):
        ...
        self.classes = (
            ...
            'tvmonitor')
        '''
        self.classes = ('__background__','ignored_regions','pedestrian', 'people', 'bicycle', 'car', 'van', 'truck', 'tricycle', 'awning-tricycle', 'bus', 'motor', 'others')

        self.num_classes = len(self.classes)
        #assert 'VOCdevkit' in devkit_path, 'VOCdevkit path does not exist: {}'.format(
        #    devkit_path)
        self.data_path = os.path.join(devkit_path, 'VOC' + year)
        self.annotaions_path = os.path.join(self.data_path, 'Annotations/xml')
    ...
    def _save_json_file(self, filename=None, data=None):
        ...
        :return
        """
        json_path = os.path.join(self.data_path, 'cocoformatJson_new')
